...
score_t MidAlphaBeta(SearchTree *tree, score_t alpha, score_t beta, unsigned char depth, bool passed)
{
    ...
    else
    {
        ...
        {
            EvaluateMoveList(tree, &moveList, tree->stones, alpha, depth, hashData);
...
score_t MidPVS(SearchTree *tree, const score_t in_alpha, const score_t in_beta, const unsigned char depth, const bool passed)
{
    ...
    { // 着手可能なとき
        ...
        // 着手の事前評価
        EvaluateMoveList(tree, &moveList, tree->stones, alpha, depth, hashData); //TODO** NULLに一時置き換えーテスト用
...
uint8 MidRoot(SearchTree *tree, bool choiceSecond)
{
    ...
    CreateMoveList(&moveList, tree->stones);
    EvaluateMoveList(tree, &moveList, tree->stones, SCORE_MIN, 0, NULL); // 着手の事前評価
...
uint8 MidRootWithMpcLog(SearchTree *deepTree, SearchTree *shallowTree, FILE *logFile, int matchIdx, uint8 shallow, uint8 deep, uint8 minimumDepth)
{
    ...
    EvaluateMoveList(deepTree, &moveList, deepTree->stones, SCORE_MIN, 0, NULL);       // 着手の事前評価
    EvaluateMoveList(shallowTree, &moveList, shallowTree->stones, SCORE_MIN, 0, NULL); // 着手の事前評価
    assert(moveList.nbMoves > 0);
